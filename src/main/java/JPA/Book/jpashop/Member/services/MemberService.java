...
// 생성자 주입을 통한 스프링 빈 등록 및 MockTest 가능
    ...
    public void validateDuplicateMember(String memberName) {
        //List<Member> byName = memberRepository.findMemberByName(member.getName());//id 검사
        List<Member> byName = memberJPARepository.findByName(memberName);

        if (!byName.isEmpty()) {
            throw new IllegalStateException("존재하는 회원 ID 입니다. " + byName);
        }
